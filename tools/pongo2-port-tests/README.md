...
Gridea Pro 主题开发 Skill 提供的 `pongo2check` 是**强制语法质量门禁**（真 Pongo2 编译，零假阳性），但它只检查语法可编译，不检查变量名、字段名、陷阱模式等**内容正确性**。本工具补上这一层——基于源码文本断言，验证迁移后的模板是否正确使用了 Gridea Pro 的变量名和 Pongo2 语法模式。
